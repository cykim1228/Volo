...
1. **[Releases](../../releases)에서 최신 `Volo-<버전>-windows.zip` 다운로드** — "Assets" 아래의 zip 을 받습니다(원하는 버전 태그 `vX.Y.Z` 선택 가능).
2. **압축 해제** — 받은 zip 을 폴더에 풉니다(예: `바탕화면\Volo`). 폴더 안 파일들이 함께 있어야 동작하니 **폴더째** 풉니다.
3. **`Volo.exe` 더블클릭** — 실행하면 앱 창이 뜹니다.
   - SmartScreen("Windows의 PC 보호") 경고가 뜨면 **추가 정보 → 실행**(서명되지 않은 배포본이라 처음 한 번 뜰 수 있습니다).
4. 영상 파일을 창에 **드래그앤드롭**.
5. 옵션(모델 크기·언어·포맷) 확인 후 **[자막 생성]**.
6. **(최초 1회) 모델 자동 다운로드** — "모델 다운로드 중 (최초 1회) … N%". `large-v3` 약 3GB / `medium` 약 1.5GB 라 **인터넷·시간 필요**하고, 멈춘 듯 보여도 받는 중입니다. 한 번 받으면 캐시되어 다음부터는 빠르고 **오프라인** 동작.
7. 진행률 대기 → 자막 미리보기(표에서 더블클릭해 수정).
8. **[편집본 내보내기]** → `.srt` 저장 → [프리미어로 가져오기](#-프리미어-프로로-자막-가져오기).
